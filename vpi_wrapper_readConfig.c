 ...
 *     #include "vpi_wrapper_readConfig.h"
 *     void (*vlog_startup_routines[])() = { readConfig_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_readConfig call=readConfig_calltf size=32 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_readConfig vpiSysFuncSized 32 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
PLI_INT32 readConfig_calltf(PLI_BYTE8 *user_data)
{
  vpiHandle hCall;
  unsigned int arg_1;
...
/* sft: $imported_readConfig vpiSysFuncSized 32 unsigned */

/* tab: $imported_readConfig call=readConfig_calltf size=32 acc=rw:%TASK */

PLI_INT32 readConfig_sizetf(PLI_BYTE8 *user_data)
{
  return 32u;
}

/* VPI wrapper registration function */
void readConfig_vpi_register()
{
  ...
  tf_data.tfname = "$imported_readConfig";
  tf_data.calltf = readConfig_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = readConfig_sizetf;
  tf_data.user_data = "$imported_readConfig";
